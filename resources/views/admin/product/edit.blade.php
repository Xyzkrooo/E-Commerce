        ...
        <div class="card-body">
            <form action="{{ route('product.update', $product->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="desc" class="form-control @error('desc') is-invalid @enderror" placeholder="Description">{{ old('desc', $product->desc) }}</textarea>
                    @error('desc')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
            ...
            init: function() {
                @if (isset($product) && $product->image)
                    ...
                    for (var i in files) {
                        var file = files[i];
                        var mockFile = {
                            name: file.image_product,
                            size: 123456
                        }; // Sesuaikan ukuran file jika diperlukan
                        this.emit("addedfile", mockFile);
                        this.emit("thumbnail", mockFile, "{{ asset('storage/products/') }}" + '/' + file
                            .image_product);
